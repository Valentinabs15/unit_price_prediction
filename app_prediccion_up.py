...
/* ===== Title & Subtitle badges for perfect readability ===== */
.title-badge {
  display: inline-block;
  padding: 10px 16px;
  border-radius: 10px;
  background: linear-gradient(90deg,#0b61ff 0%, #0a3d98 100%);
  color: #fff !important;
  font-weight: 900;
  font-size: 40px;
  line-height: 1.1;
...
with st.container():
    ...
    st.markdown('<h1 class="title-badge">💎 Predicción de Precio Unitario</h1>', unsafe_allow_html=True)
    st.markdown('<div class="subtitle-badge">Simula escenarios por zona y volumen con variables macroeconómicas</div>', unsafe_allow_html=True)
