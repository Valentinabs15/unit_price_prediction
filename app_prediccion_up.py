...
def set_background():
    ...
    st.markdown("""
        <style>
        .stApp { background: linear-gradient(180deg,#f5f6fa 0%,#fff 60%); }
        </style>
    """, unsafe_allow_html=True)
...
/* Texto GLOBAL dentro de la card en BLANCO con máxima prioridad */
.hero *, 
.hero [data-testid="stMarkdownContainer"], 
.hero [data-testid="stMarkdownContainer"] *, 
.hero [data-testid="stWidgetLabel"], 
.hero [data-testid="stWidgetLabel"] *,
.hero label, .hero p, .hero span, .hero small, .hero strong, .hero h1, .hero h2, .hero h3, .hero h4 {
  color: #ffffff !important;
  opacity: 1 !important;
  text-shadow: 0 1px 2px rgba(0,0,0,.85) !important;
}

/* Inputs blancos con texto negro (sin sombra) */
...
.hero .stNumberInput input { color: #111 !important; text-shadow: none !important; }
.hero .stTextInput input { color: #111 !important; text-shadow: none !important; }
.hero .stSelectbox > div > div { color: #111 !important; text-shadow: none !important; }

/* Botón centrado y marcado */
