...
# ============================
# Setup y estilo (hero + overlay legible)
# ============================
...
# Hero/card styles con alto contraste
st.markdown("""
<style>
.hero {
...
.hero h1 { 
  font-size: 44px; line-height: 1.1; margin: 4px 0 10px 0; 
  color: #111; text-shadow: 0 1px 1px rgba(255,255,255,0.6);
}
.hero p.sub { color: #222; margin-bottom: 18px; font-size: 16px; }
</style>
""", unsafe_allow_html=True)
...
def cargar_modelos_desde_raiz():
    ...
    # Clusterización automática (requiere kmeans + scaler_cluster)
...
def predecir_varios_periodos(volume, zona, lista_valores, modelos, kmeans=None, scaler_cluster=None, cluster_manual=None):
    ...
    if (kmeans is not None) and (scaler_cluster is not None):
        ...
        if cluster_manual is None:
            raise RuntimeError("Faltan kmeans.pkl o scaler_cluster.pkl. Sube ambos a la raíz del repo para clasificar automáticamente.")
...
with st.container():
    ...
    if modelos_ok and (kmeans is None or scaler_cluster is None):
        st.warning("Para **clasificación automática de clúster** necesitas **kmeans.pkl** y **scaler_cluster.pkl** en la raíz del repositorio.")
        cluster_manual = st.selectbox("⚠️ Temporal: elige modelo por clúster", options=[0,1,2,3],
